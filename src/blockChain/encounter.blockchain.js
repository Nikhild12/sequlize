...
const encounterMasterBlockChain = () => {
  ...
  const _queryStringEncounterBlockChain = async (encounterObject) => {
    const encounterCreateUrl = emr_utility.deployedBlockChainUrl() + `${BLOCK_CHAIN_URL.ENOCUNTER_QUERY_STRING}`;
    const encounterCreateObjects = emr_utility.postRequest(encounterCreateUrl, { Authorization: TOKEN }, { "selector": encounterObject });
    return encounterCreateObjects;
  };
  ...
  return {
    ...
    queryStringEncounterBlockChain: _queryStringEncounterBlockChain
  };
};
