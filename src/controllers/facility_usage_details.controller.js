...
const getfacilityusagedetails = async (req, res, next) => {
  try {
    ...
    // " SUM(CASE WHEN oecc.is_adult = 0 AND oecc.gender_uuid = 3 AND oecc.encounter_visit_type_uuid != 2 THEN 1 ELSE 0 END) AS new_child_transgender," +  // H30-50026-khurshid--Institution Wise performance Reports (Grand total Issue)
    " SUM(CASE WHEN oecc.is_adult = 0 AND oecc.encounter_visit_type_uuid != 2 AND oecc.gender_uuid != 3 THEN 1 ELSE 0 END) AS new_child_total," +  // H30-50026-khurshid--Institution Wise performance Reports (Grand total Issue)
    " SUM(CASE WHEN oecc.encounter_visit_type_uuid != 2 THEN 1 ELSE 0 END) - SUM(CASE WHEN oecc.is_adult = 0 AND oecc.gender_uuid = 3 AND oecc.encounter_visit_type_uuid != 2 THEN 1 ELSE 0 END) AS total_new_patients," +  // H30-50026-khurshid--Institution Wise performance Reports (Grand total Issue)
    ...
    " SUM(CASE WHEN oecc.encounter_visit_type_uuid != 2 THEN 1 ELSE 0 END) + SUM(CASE WHEN oecc.encounter_visit_type_uuid = 2 THEN 1 ELSE 0 END) - SUM(CASE WHEN oecc.is_adult = 0 AND oecc.gender_uuid = 3 AND oecc.encounter_visit_type_uuid != 2 THEN 1 ELSE 0 END)AS total_patients," +  // H30-50026-khurshid--Institution Wise performance Reports (Grand total Issue)
