      ...
      if (user_uuid > 0 && procedureNoteTemplatesData) {
        ...
        try {
          ...
          if (tblname_exits && tblname_exits.length > 0) {
            return res
              .status(422)
              .send({ statusCode: 422, message: "procedure_uuid and display_order already exists" });
          }
          else if (code_exits && code_exits.length > 0) {
            return res
              .status(422)
              .send({ statusCode: 422, message: "procedure_uuid already exists" });

          } else if (name_exits && name_exits.length > 0) {
            return res
              .status(422)
              .send({ statusCode: 422, message: "display_order already exists" });
