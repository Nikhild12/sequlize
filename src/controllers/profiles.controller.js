...
const profilesController = () => {
  ...
  const _getAllProfiles1 = async (req, res, next) => {
    try {
      const { profile_type } = req.query;
      const getsearch = req.body;
      let pageNo = 0;
      ...
      const data = await profilesViewTbl.findAndCountAll(findQuery)
      // console.log('data==', data);
      if (data) {
        // console.log('profiletypeid==', data.rows[0].p_profile_type_uuid);
        // if (data.rows[0].p_profile_type_uuid == 5) {
        //   return res.status(200).send({ code: httpStatus.OK, message: emr_constants.FETCHD_PROFILES_SUCCESSFULLY, responseContents: data });
        // }
        // else {
    ...
    // if (data) {
    //   if (data.p_profile_type_uuid == 1) {
    //     vdata = getventilatorData(data);
    //     return res.status(httpStatus.OK).json({ statusCode: 200, req: '', responseContents: vdata });
    //   } else {
    //     return res.status(400).send({ code: httpStatus[400], message: "no data found" });
    //   }
    // }
    //     .then((data) => {p_profile_type_uuid
    //       return res
    //         .status(httpStatus.OK)
    //         .json({
    //           statusCode: 200,
